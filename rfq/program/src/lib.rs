...
pub mod rfq {
    ...
    pub fn create_rfq<'info>(
        ...
        // Store which token program we're using
        rfq.token_program = ctx.accounts.token_program.key();
        
        // Validate token program
        common::validate_token_program(&ctx.accounts.token_program)?;
...
pub struct CreateRfqAccounts<'info> {
    ...
    /// CHECK: Can be either SPL Token or Token-2022 program
